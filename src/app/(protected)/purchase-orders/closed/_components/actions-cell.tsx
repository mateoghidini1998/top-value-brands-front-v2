...
const ActionsCell = ({ orderId, missingProducts }: ActionsCellProps) => {
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="relative">
            ...
            {missingProducts && (
              <AlertCircle className="absolute top-0 right-0 h-3 w-3 stroke-red-500" />
            )}
          </div>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="dark:bg-table_header">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem>
            <Link href={`closed/${orderId}`}>View Details</Link>
          </DropdownMenuItem>
